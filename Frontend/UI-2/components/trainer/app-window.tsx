...
import { SignTalkMark } from '@/components/trainer/signtalk-lockup'
import { ThemeToggle } from '@/components/trainer/theme-toggle'
      ...
      <div className="flex w-full flex-col overflow-hidden rounded-2xl bg-card text-card-foreground shadow-window">
        <header className="relative flex h-11 shrink-0 items-center gap-3 border-b bg-secondary/70 px-3 backdrop-blur-sm sm:px-4">
          ...
          <div className="flex min-w-0 flex-1 items-center justify-center gap-2">
            <SignTalkMark className="size-4" />
            <span className="truncate text-xs font-medium tracking-tight text-muted-foreground">{title}</span>
          </div>
        ...
        <div className="flex min-h-[32rem] flex-1 flex-col sm:min-h-[36rem] lg:min-h-[38rem]">{children}</div>
      </div>
    </div>
  )
}
