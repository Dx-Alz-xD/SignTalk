"""SQL-backed account store - PostgreSQL or SQLite, whichever backend/db.py
picked.
...
def _user_columns() -> str:
    # email/username are citext on Postgres; read them back as plain text.
    return f"""
        id, {db.dialect.text('email')} AS email,
        {db.dialect.text('username')} AS username, phone, password_hash,
...
class SqlStore:
    ...
    def get_challenge(self, challenge_id: str) -> OtpChallenge | None:
        row = db.fetch_one(
            f"""
            SELECT c.*, {db.dialect.text('u.email')} AS email
    ...
    def get_ticket(self, ticket_id: str) -> RecoveryTicket | None:
        row = db.fetch_one(
            f"""
            SELECT t.*, {db.dialect.text('u.email')} AS email
    ...
    def get_session(self, token: str) -> Session | None:
        row = db.fetch_one(
            f"""
            SELECT s.*, {db.dialect.text('u.email')} AS email,
                   {db.dialect.text('u.username')} AS username
    ...
    def extend_session(self, token: str, expires_at) -> None:
        """Push a live session's expiry out. See AuthService.resolve_session."""
        db.execute(
            "UPDATE sessions SET expires_at = %s "
            " WHERE token_hash = %s AND revoked_at IS NULL",
            (expires_at, db.token_digest(token)),
        )
...
# The class used to be Postgres-only and was imported under this name.
PostgresStore = SqlStore
