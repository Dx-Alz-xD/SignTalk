...
# 30 minutes, matching AuthService.SESSION_TTL_MINUTES. The cookie must not
# outlive the row, or the browser keeps sending a token the server has binned.
COOKIE_MAX_AGE = AuthService.SESSION_TTL_MINUTES * 60


@lru_cache(maxsize=1)
def get_auth() -> AuthService:
    """One service for the process, over the Postgres store.

    seed_demo_account stays on so example@gmail.com / 123456 works against a
    fresh database - the same account the CLI seeds.
    """
    return AuthService(store=PostgresStore(), seed_demo_account=True)
